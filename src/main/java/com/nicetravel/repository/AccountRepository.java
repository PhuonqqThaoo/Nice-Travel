...
//    List<Account> findByIsEnable (Boolean isDeleted) ;
    ...
    @Query(value ="UPDATE Account SET fullname =?1, email = ?2, phone = ?3, idCard= ?4,  gender =?5 WHERE username =?6", nativeQuery = true)
    void updateNonPass(String fullname, String email, String phone,String idCard, Boolean gender, String username);
    
    @Modifying(clearAutomatically =true)
    @Query(value="UPDATE Account SET fullname =?1, email = ?2,password=?3, phone = ?4, idCard= ?5,  gender =?6 WHERE username =?7", nativeQuery = true)
    void update (String fullname, String email,String password, String phone,String idCard, Boolean gender, String username);
